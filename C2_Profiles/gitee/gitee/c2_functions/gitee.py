...
class Gitee(C2Profile):
    name = "gitee"
    description = "Gitee C2 Profile"
    is_p2p = False
    is_server_routed = False
    server_folder_path = Path(".") / "gitee" / "c2_code"
    server_binary_path = server_folder_path / "server.py"
    parameters = [
        C2ProfileParameter(
            name="gitee_repo",
            description="Gitee repository to send communication through",
            default_value="",
            required=True,
        ),
        C2ProfileParameter(
            name="gitee_username",
            description="Gitee Username that owns the repo",
            default_value="",
            required=True,
        ),
        C2ProfileParameter(
        ...
        C2ProfileParameter(
            ...
            description="Issue # that Mythic will post jobs to",
            default_value="1",
            required=True,
        ),
        C2ProfileParameter(
            name="client_issue_number",
            description="Issue # that agents will post results to",
            default_value="2",
            required=True,
        ),
        C2ProfileParameter(
